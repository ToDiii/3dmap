<script lang="ts">
	...
	$: points = (() => {
		...
		return elevations.map((e, i) => `${i * dx},${height - (e - min) * dy}`).join(' ');
	})();
	...
	function handleMove(e: MouseEvent) {
		...
		tooltip = {
			...
			y:
				height -
				((elev - Math.min(...elevations)) * height) /
...
{#if tooltip}
	<div class="text-xs">
		Distanz: {(tooltip.dist / 1000).toFixed(2)} km, Höhe: {tooltip.elev.toFixed(0)} m, Steigung: {tooltip.slope.toFixed(
			1
		)}%
	</div>
{/if}
