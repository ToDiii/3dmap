...
<div class="space-y-2">
	<button class="w-full bg-blue-600 p-2 text-white" on:click={exportSTL}> Export als STL </button>
	<button class="w-full bg-blue-600 p-2 text-white" on:click={export3MF}> Export als 3MF </button>
	{#if exportMessage}
		<div class="mt-2 text-sm">{exportMessage}</div>
	{/if}
</div>
