...
<div class="space-y-4">
	{#each $routeStore.waypoints as wp, i (wp.id)}
		<div class="relative">
			<input
			...
			{#if suggestions[wp.id]?.length}
				<ul class="absolute z-10 max-h-40 w-full overflow-auto border bg-white">
					{#each suggestions[wp.id] as s (s.label)}
						<li>
							<button
								type="button"
			...
			<div class="mt-1 flex space-x-1">
				<button on:click={() => routeStore.removeWaypoint(wp.id)} class="bg-red-500 px-1 text-white"
					>X</button
				>
				<button
					on:click={() => routeStore.reorder(i, i - 1)}
					disabled={i === 0}
					class="bg-gray-300 px-1"
				>
					↑
				</button>
				<button
	...
	{#if $routeStore.waypoints.length < ROUTING_MAX_WAYPOINTS}
		<button class="bg-blue-500 px-2 py-1 text-white" on:click={routeStore.addWaypoint}
	...
	{/if}
	<div class="space-x-2">
		<button class="bg-green-600 px-2 py-1 text-white" on:click={calculateRoute} disabled={routing}>
			Route berechnen
		</button>
		<button
			class="bg-purple-600 px-2 py-1 text-white"
			on:click={exportGpx}
