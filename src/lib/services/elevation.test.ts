...
describe('sampleElevation batching', () => {
  it('batches requests', async () => {
    ...
    const fetchMock = vi.fn((url, opts: any) => {
      const body = JSON.parse(opts.body);
      const res = body.locations.map((_: any, i: number) => ({ elevation: responses.shift() }));
      return Promise.resolve(
        new Response(JSON.stringify({ results: res }), { status: 200 })
      );
    });
